...
int CountLinesInFile(const CString& filePath)
{
	CStdioFile file;
...
void WinMTRDialog::SaveDataListToFile(const std::list<data>& datalist, const CString& folderPath)
{
	...
	timeStream << std::put_time(&now_tm, "%Y_%m_%d_%H_%M_%S");
	CString currentTime(timeStream.str().c_str());

	// Create a base file name with date and time
	CString baseFilePath;
	baseFilePath.Format(_T("%s\\datalist_%s"), folderPath, currentTime);

	CString filePath;
	int fileIndex = 0;
	do
	{
		filePath.Format(_T("%s_%d.txt"), baseFilePath, fileIndex);
		++fileIndex;
	} while (CountLinesInFile(filePath) >= MAXLINE);

	CStdioFile file;
	CFileException fileException;
	bool isNewFile = !PathFileExists(filePath);
	if (!file.Open(filePath, CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite | CFile::typeText, &fileException))
	{
	...
	for(int i=0; i <nh ; ++i) {
		...
		if (datalist.size() >= 1000)
		{
			...
			if (!exeDir.IsEmpty())
			{
				...
				{
					AfxMessageBox(_T("Failed to create directory in executable folder."));
				}
			}
		}
	}
